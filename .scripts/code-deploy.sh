...
if [ -z "$GITHUB_REPOSITORY" ]; then
    error "Please set the \"\$GITHUB_REPOSITORY\" variable"
    exit 1
fi
...
DEPLOYMENT_CMD="aws deploy create-deployment $IGNORE_APPLICATION_STOP_FAILURES_SWITCH --output json --application-name $APPLICATION_NAME --deployment-config-name $DEPLOYMENT_CONFIG_NAME --deployment-group-name $DEPLOYMENT_GROUP --github-location $GITHUB_LOCATION"
